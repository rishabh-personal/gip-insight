...
export interface ZwingJobStatusResult {
  invoiceIds: string[];
  /** per-invoice rollup: pass if ANY connector succeeded */
  byInvoice: Map<string, { hasSuccess: boolean; hasAnyJob: boolean }>;
...
export class ZwingStatusService {
  ...
  async buildZwingJobStatus(
    ...
    // Roll up to invoice level: success if ANY connector delivered successfully
    const byInvoice = new Map<string, { hasSuccess: boolean; hasAnyJob: boolean }>();
    for (const id of invoiceIds) byInvoice.set(id, { hasSuccess: false, hasAnyJob: false });
    ...
    const byPair = jobGroups.map((g) => ({
      ...
      // "purely failed" = failed attempts exist AND no success for this connector
      hasFailed:      !!g.hasFailed && !g.hasSuccess,
